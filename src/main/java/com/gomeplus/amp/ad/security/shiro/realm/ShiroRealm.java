...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		try {
			merchantUserLoginService.setName(name);
			merchantUserLoginService.setPassword(password);
			merchantUserLoginService.setUserType(1);
			merchantUserLoginService.execute();
		} catch (Exception e) {
			throw new AuthenticationException(e.getMessage());
		}

		if (merchantUserLoginService.getExtUserId() == null) {
			throw new AuthenticationException("服务器繁忙");
		}

		User user = new User();
		user.setExtUserId(merchantUserLoginService.getExtUserId());
		user.setLoginName(name);
		user.setNickname(merchantUserLoginService.getNickName());
		user.setName(merchantUserLoginService.getNickName());
		user.setMobile(merchantUserLoginService.getMobile());
		user.setEmail(merchantUserLoginService.getEmail());
		user.setAvatar(merchantUserLoginService.getFacePicUrl());
		user.setRegisterTime(new Date(merchantUserLoginService.getRegisterTime()));
		user.setType(merchantUserLoginService.getType());
